...
def create_invoice(company, customers, invoice_number, invoice_date, manager):
    ...
    for customer in customers:
        ...
        # Customer Subtotal (fixed formatting)
        c.setFont("Helvetica-Bold", 12)
...
def modify_invoice(manager):
    ...
    try:
        ...
        print("Enter new details (press Enter at customer name to keep existing customers and items):")
        
        # Get new customer details and items
        customers = get_customer_details()
        if not customers:
            customers = existing_invoice["customers"]
        
        # Get new invoice date
        ...
        # Reset invoice number counter if necessary
